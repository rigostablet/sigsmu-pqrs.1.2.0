 ...
 */
 $(document).on('submit', 'form.ajax-pagination-form', function(event){
   ...
   //Oculta campos de la tabla pqrspet dependiendo del control
   $(document).ready(function() { //prepara el documento a trabajar
        ...
        $("#ctrl-id_ent").on('change', function() { 
            if ($("#ctrl-id_ent").val()!=1) { //para verificar el valor del campo
...
//Colorea las filas dependiendo el valor del campo diasPen -->
$(document).ready(function() { //prepara el documento a trabajar            
...
//------------------------------------------------------------------------------------------
// Oculta el campo "nit" de la tabla pqrsent si el campo seleccionado es
$(document).ready(function() { //prepara el documento a trabajar
        // Ocultar los elementos al cargar la página
        $("#ctrl-nit_ent").show();//muestra el campo nombre peticionario
        $("label[for='nit_ent']").show();// muestra la etiqueta del campo nombre peticionario
        $("#ctrl-tip_ent").on('change', function() { 
            if ($("#ctrl-tip_ent").val()==3) { //condicional que verifica el valor del campo
                $("#ctrl-nit_ent").hide(); // oculta el campo si el valor es  igual a 2            
                $("label[for='nit_ent']").hide();// oculat la etiqueta
                } else {
                $("#ctrl-nit_ent").show(); // muestra el campo            
                $("label[for='nit_ent']").show();// muestra la etiqueta
            }
        });
    });    
//Toma el texto seleccionado en el select y lo pega en el campo cod_mun. Tabla pqrs_mun
$(document).ready(function() {
  // Escucha el cambio en el campo "cod_mun"
  $("#ctrl-cod_mun").change(function() {
    // Obtiene el valor seleccionado
    let textoSeleccionado = $("#ctrl-cod_mun option:selected").text();
    // Asigna el valor al campo "código"
    $("#ctrl-nom_mun").val(textoSeleccionado);
  });
});
//===============================================================================
//Toma el texto seleccionado en el select y lo pega en el campo ofic_mun. Tabla User
$(document).ready(function() {
  // Escucha el cambio en el campo "cod_mun"
  $("#ctrl-oficina").change(function() {
    // Obtiene el valor seleccionado
    let textoSelected = $("#ctrl-oficina option:selected").text();
    // Asigna el valor al campo "código"
    $("#ctrl-nom_ofic_user").val(textoSelected);
  });
});
//Toma el texto seleccionado en el select municipio y lo pega en el campo cod_mun. Tabla User
$(document).ready(function() {
  // Escucha el cambio en el campo "cod_mun"
  $("#ctrl-cod_mun").change(function() {
    // Obtiene el valor seleccionado
    let textoSelected = $("#ctrl-cod_mun option:selected").text();
    // Asigna el valor al campo "código"
    $("#ctrl-mun").val(textoSelected);
  });
});
