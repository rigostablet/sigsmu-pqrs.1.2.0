...
class ComponentsData{
	...
	/**
     * dpto_mun_option_list Model Action
     * @return array
     */
	function dpto_mun_option_list(){
		$sqltext = "SELECT  DISTINCT cod_dpto_pqrs AS value,nom_dpto_pqrs AS label FROM pqrs_dpto ORDER BY nom_dpto_pqrs";
		$query_params = [];
		$arr = DB::select($sqltext, $query_params);
		return $arr;
	}
	...
	/**
     * Check if value already exist in PqrsMun table
	 * @param string $value
     * @return bool
     */
	function pqrsmun_cod_mun_value_exist(Request $request){
		$value = trim($request->value);
		$exist = DB::table('pqrs_mun')->where('cod_mun', $value)->value('cod_mun');   
		if($exist){
			return true;
		}
		return false;
     ...
     * @return array
     */
	function cod_mun_option_list($value = null){
		$lookup_value = request()->lookup ?? $value;
		$sqltext = "SELECT  DISTINCT cod_mun AS value,nom_mun AS label FROM pqrs_dpto WHERE cod_dpto_pqrs=:lookup_dpto_mun ORDER BY nom_mun" ;
		$query_params = [];
		$query_params['lookup_dpto_mun'] = $lookup_value;
		$arr = DB::select($sqltext, $query_params);
		return $arr;
	}
	...
	/** VALIDACION DE CAMPO pet_numpet DE TABLA pqrs_pet */
	/**
	...
	/**
     * dpto_pet_option_list Model Action
     * @return array
     */
	function dpto_pet_option_list(){
		$sqltext = "SELECT  DISTINCT cod_dpto_pqrs AS value,nom_dpto_pqrs AS label FROM pqrs_dpto";
		$query_params = [];
		$arr = DB::select($sqltext, $query_params);
		return $arr;
	}
	...
	/**
     * mun_pet_option_list Model Action
     * @return array
     */
	function mun_pet_option_list($value = null){
		$lookup_value = request()->lookup ?? $value;
		$sqltext = "SELECT  DISTINCT cod_mun AS value,nom_mun AS label FROM pqrs_dpto WHERE cod_dpto_pqrs=:lookup_dpto_pet" ;
		$query_params = [];
		$query_params['lookup_dpto_pet'] = $lookup_value;
		$arr = DB::select($sqltext, $query_params);
		return $arr;
	}
	...
	function id_tip_ent_option_list(){
		$sqltext = "SELECT  DISTINCT id_tip AS value,tipent_nom AS label FROM pqrs_tipent WHERE id_tip = 3";
		$query_params = [];
		$arr = DB::select($sqltext, $query_params);
		return $arr;
	}
     ...
     * @return array
     */
	function id_ent_resp_option_list(){
		$sqltext = "SELECT  DISTINCT id_ent AS value,nom_ent AS label FROM pqrs_ent ORDER BY nom_ent";
		$query_params = [];
		$arr = DB::select($sqltext, $query_params);
		return $arr;
	}
	
	/**ESTE ES EL CODIGO DEL CONTROL DPTO   */
	/**
     * dpt_resp_option_list Model Action
     * @return array
     */
	function dpt_resp_option_list(){
		$sqltext = "SELECT  DISTINCT nom_dpto_pqrs AS value,nom_dpto_pqrs AS label FROM pqrs_dpto ORDER BY nom_dpto_pqrs";
		$query_params = [];
		$arr = DB::select($sqltext, $query_params);
		return $arr;
	}	
	...
	function rad_respu_option_list(){
		...
		$sqltext = "SELECT  pqrs_reg_pqrs.rad_sol AS value FROM pqrs_reg_pqrs WHERE  regsol_est  ='ASIGNADO' AND pqrs_reg_pqrs.ofic_usu_sol = $ofic_logue";
		$query_params = [];
		$arr = DB::select($sqltext, $query_params);
		return $arr;
	}
	...
	/** CHEQUEO SI EL DATOS YA EXISTE */
	/**
     * chequea si existe el numero de documento
	 * @param string $value
     * @return bool
     */
	function pqrs_pet_pet_numpet_value_exist(Request $request){
		
		$value = trim($request->value);
		$exist = DB::table('pqrs_pet')->where('pet_numpet', $value)->value('pet_numpet');   
		if($exist){
			return true;
		}
		return false;
	...
	/**
     * user_cod_mun_option_list Model Action
     * @return array
     */
	function user_cod_mun_option_list(){
	...
	function getcount_sinnotificar(){
		...
		$sqltext = "SELECT COUNT(*) AS num FROM pqrs_reg_pqrs where regsol_est = 'SIN ASIGNAR' and  ofic_usu_sol = $ofic_logue AND mun_user_sol = $mun_logue" ;
		$query_params = [];
	...
	function getcount_sinresponder(){
		$ofic_logue = Auth()->user()->oficina; // Obtenemos la instancia del usuario logueado
		$mun_logue = Auth()->user()->cod_mun; // Obtenemos la instancia del usuario logueado
		$sqltext = "SELECT COUNT(*) AS num FROM pqrs_reg_pqrs where regsol_est = 'ASIGNADO' and  ofic_usu_sol = $ofic_logue AND mun_user_sol =$mun_logue";
		$query_params = [];
		$val = DB::selectOne($sqltext, $query_params);
		return $val->num;
	}
	...
	/**
     * getcount_respuestas Model Action
     * @return int
     */
	function getcount_respuestas(){
		$ofic_logue = Auth()->user()->oficina; // Obtenemos la instancia del usuario logueado
		$mun_logue = Auth()->user()->cod_mun; // Obtenemos la instancia del usuario logueado
		$sqltext = "SELECT COUNT(*) AS num FROM pqrs_reg_pqrs where regsol_est = 'RESUELTA' and  ofic_usu_sol = $ofic_logue AND mun_user_sol =$mun_logue";
		$query_params = [];
		$val = DB::selectOne($sqltext, $query_params);
		return $val->num;
	}
	...
	function getcount_totalpqrs(){
		$ofic_logue = Auth()->user()->oficina; // Obtenemos la instancia del usuario logueado
		$mun_logue = Auth()->user()->cod_mun; // Obtenemos la instancia del usuario logueado
		$sqltext = "SELECT COUNT(*) AS num FROM pqrs_reg_pqrs where ofic_usu_sol = $ofic_logue AND mun_user_sol =$mun_logue";
		$query_params = [];
		$val = DB::selectOne($sqltext, $query_params);
		return $val->num;
	}
